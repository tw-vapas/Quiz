...
export default function FileManager() {
  ...
  const [newFileDataOption, setNewFileDataOption] = useState<"BLANK" | "IMPORT_FILE" | "MERGE">("BLANK");
  ...
  const handleCreateFile = () => {
    ...
    } else if (newFileDataOption === "IMPORT_FILE" && importedData) {
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg overflow-hidden transition-all duration-300 shadow-sm relative">
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-md transition-all duration-300">
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-2xl space-y-5 flex flex-col max-h-[90vh] overflow-hidden relative">
            {/* Modal Header */}
            ...
            <div className="flex-1 overflow-y-auto space-y-5 pr-1 custom-scrollbar">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className={cn(
                      "py-2 px-3 rounded-xl border text-sm font-semibold transition-all cursor-pointer select-none text-center",
                  ...
                  <button
                    type="button"
                    onClick={() => setNewFileDataOption("IMPORT_FILE")}
                    className={cn(
                      "py-2 px-3 rounded-xl border text-sm font-semibold transition-all cursor-pointer select-none text-center",
                      newFileDataOption === "IMPORT_FILE"
                        ? "border-indigo-600 bg-indigo-50/50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 font-bold shadow-xs"
                        : "border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-700"
                    )}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setNewFileDataOption("MERGE")}
                    className={cn(
                      "py-2 px-3 rounded-xl border text-sm font-semibold transition-all cursor-pointer select-none text-center",
              ...
              {/* Display Block 1: Dữ liệu từ máy */}
              {newFileDataOption === "IMPORT_FILE" && (
                <div 
                  onClick={() => !isParsing && fileInputRef.current?.click()}
                  className={cn(
                    "border-2 border-dashed rounded-xl p-5 text-center transition-all flex flex-col items-center justify-center w-full min-h-[130px]",
                    isParsing 
                ...
                >
                  ...
                  {isParsing ? (
                    ...
                    <>
                      <div className="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 flex items-center justify-center mb-2 group-hover/drop:scale-110 transition-transform">
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 shrink-0 flex gap-3">
              <button 
                ...
                disabled={isParsing}
                className={cn(
                  "flex-1 py-2.5 border rounded-xl font-semibold text-xs transition-all cursor-pointer",
                  isParsing
              ...
              <button 
                ...
                className={cn(
                  "flex-1 py-2.5 font-bold text-xs rounded-xl shadow-sm transition-all flex items-center justify-center gap-1.5 cursor-pointer",
